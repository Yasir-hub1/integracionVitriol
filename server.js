...
// Middleware - LÍMITES AUMENTADOS
app.use(cors());
app.use(
  express.json({
    limit: "50mb", // Aumentado a 50MB para archivos grandes
...
app.use(
  express.urlencoded({
    extended: true,
    limit: "50mb", // También para datos URL-encoded
    parameterLimit: 50000,
  })
);
...
// 📨 ENDPOINT PRINCIPAL - PROCESAR HTML Y GUARDAR EN DB (MEJORADO)
app.post("/api/process-file", async (req, res) => {
  const client = await pool.connect();

  try {
    const { filename, content, size, modified, encoding } = req.body;

    console.log(`📄 Procesando archivo: ${filename}`);
    console.log(
      `📏 Tamaño del archivo: ${size} bytes (${(size / 1024 / 1024).toFixed(
        2
      )} MB)`
    );
    console.log(`📝 Codificación detectada: ${encoding || "no especificada"}`);
    console.log(`📏 Tamaño del contenido HTML: ${content.length} caracteres`);

    // Validar tamaño máximo permitido (50MB)
    if (content.length > 50 * 1024 * 1024) {
      return res.status(413).json({
        success: false,
        message: "Archivo demasiado grande. Máximo permitido: 50MB",
        fileSize: size,
        maxAllowed: 50 * 1024 * 1024,
      });
    }
    ...
    // Para archivos muy grandes, procesar en lotes
    const BATCH_SIZE = 100;
    const batches = [];
    for (let i = 0; i < ripJobs.length; i += BATCH_SIZE) {
      batches.push(ripJobs.slice(i, i + BATCH_SIZE));
    }
      ...
      for (const job of batch) {
        try {
          ...
          if (existingJob.rows.length > 0) {
            console.log(`⚠️ Trabajo duplicado: tabla ${job.table_number}`);
            duplicateJobs++;
            continue;
          }
          ...
          if (newJobs % 10 === 0) {
            console.log(`✅ ${newJobs} trabajos guardados...`);
          }
        } catch (error) {
          console.error(
            `❌ Error guardando trabajo tabla ${job.table_number}:`,
            error.message
          );
        }
      }
    ...
    console.log(
      `🎉 Procesamiento completado: ${newJobs} nuevos, ${duplicateJobs} duplicados`
    );
    ...
    res.json({
      ...
      processedJobs: processedJobs.slice(0, 100), // Primeros 100 para no sobrecargar la respuesta
    });
  } catch (error) {
...
app.get("/api/rip-jobs/:id/content", async (req, res) => {
  try {
    ...
    const result = await pool.query("SELECT * FROM rip_jobs WHERE id = $1", [
      jobId,
    ]);
    ...
    const response = {
      ...
      html_info: {
        ...
        table_html_preview: job.table_html
          ? job.table_html.substring(0, 500) + "..."
          : "No disponible",
...
app.get("/api/rip-jobs/all-content", async (req, res) => {
  try {
    ...
    console.log(
      `📊 Resultados: ${jobsResult.rows.length} trabajos de ${total} total`
    );
...
app.post("/api/rip-jobs/export-excel", async (req, res) => {
  try {
    const { startDate, endDate, workStartDate, workEndDate, printer, status } =
      req.body;
    ...
    const excelData = result.rows.map((job) => ({
      ...
      "Fecha Creación Sistema": job.created_at
      ...
      "Inicio RIP": job.rip_start_datetime
        ? new Date(job.rip_start_datetime).toLocaleString("es-ES")
        : "N/A",
      "Fin RIP": job.rip_end_datetime
        ? new Date(job.rip_end_datetime).toLocaleString("es-ES")
        : "N/A",
      "Duración RIP": job.rip_duration || "N/A",
      "Inicio Salida": job.output_start_datetime
        ? new Date(job.output_start_datetime).toLocaleString("es-ES")
        : "N/A",
      "Fin Salida": job.output_end_datetime
        ? new Date(job.output_end_datetime).toLocaleString("es-ES")
        : "N/A",
...
function extractRipJobsFromHTML(htmlContent) {
  ...
  for (let i = 0; i < tables.length; i++) {
    ...
    if (Object.keys(job).length > 3) {
      // Al menos tiene HTML + table_number + algún campo más
      console.log(
        `✅ Tabla ${i + 1}: HTML completo guardado (${
          tableHTML.length
        } chars) + ${Object.keys(extractedFields).length} campos extraídos`
      );
      jobs.push(job);
    } else {
      console.log(
        `⚠️ Tabla ${i + 1}: Solo HTML guardado, pocos campos extraídos`
      );
    ...
    if (thMatch && tdMatch) {
      ...
      if (!dbField) {
        ...
        for (const [key, value] of Object.entries(fieldMapping)) {
          if (
            fieldNameLower.includes(key.toLowerCase().replace(":", "")) ||
      ...
      if (
        ...
        if (dbField === "number_of_copies" || dbField === "number_of_pages") {
          ...
          dbField.includes("datetime") ||
          dbField === "rip_start_datetime" ||
          dbField === "rip_end_datetime"
        ) {
        ...
        const unmappedField = fieldName
          .replace(/[^a-zA-Z0-9]/g, "_")
          .toLowerCase();
        job[`unmapped_${unmappedField}`] = fieldValue;
        fieldsExtracted++;
        console.log(
          `  📝 Campo no mapeado guardado como: unmapped_${unmappedField}`
        );
      }
    }
  }
...
function parseDateTime(dateTimeStr) {
  ...
  try {
    ...
    if (!isNaN(parsedDate.getTime())) {
      console.log(
        `✅ Fecha parseada (Date.parse): ${parsedDate.toISOString()}`
      );
...
app.get("/api/rip-jobs/tables/all", async (req, res) => {
  try {
    ...
    const countResult = await pool.query(
      "SELECT COUNT(*) as total FROM rip_jobs"
    );
    const total = parseInt(countResult.rows[0].total);
...
app.get("/api/rip-jobs/:id/download-html", async (req, res) => {
  try {
    ...
    const job = result.rows[0];
    const filename = `tabla_${jobId}_${job.table_number}_${
